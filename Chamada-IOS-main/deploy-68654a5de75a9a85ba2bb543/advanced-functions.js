...
function addStudent() {
  ...
  if (students.find((s) => s.name === studentName && s.classId === classId)) {
...
function loadAttendanceStudents() {
  ...
  classStudents.forEach((student) => {
    ...
    const currentStatus = existingAttendance[student.name] || {
      status: "not-marked",
    };
...
function markAttendance(studentName, status, attendanceKey, element) {
  ...
  if (!attendance[attendanceKey][studentName]) {
    attendance[attendanceKey][studentName] = {};
  }

  attendance[attendanceKey][studentName].status = status;
...
function updateJustification(studentName, attendanceKey, isJustified) {
  if (!attendance[attendanceKey] || !attendance[attendanceKey][studentName]) {
    return;
  }

  attendance[attendanceKey][studentName].justified = isJustified;
}
...
function uploadAttachment(studentName, attendanceKey, file) {
  ...
  attendance[attendanceKey][studentName].attachment = file.name;
...
function saveAttendance() {
  ...
  if (markedStudents < classStudents.length) {
    if (
      !confirm(
        `Apenas ${markedStudents} de ${classStudents.length} estudantes foram marcados. Deseja salvar mesmo assim?`
      )
    ) {
      return;
    }
...
function loadReportClasses() {
  ...
  let filteredClasses = classes;
  if (unitId) {
    filteredClasses = classes.filter((c) => c.unitId === parseInt(unitId));
  }
...
function calculateReportData(unitId, classId, startDate, endDate) {
  ...
  if (classId) {
    filteredClasses = filteredClasses.filter((c) => c.id === parseInt(classId));
  }
  ...
  filteredClasses.forEach((cls) => {
    ...
    reportData.totalStudents += classStudents.length;
...
function exportToExcel() {
  ...
  Object.keys(attendance).forEach((key) => {
    ...
    if (date >= startDate && date <= endDate) {
      const cls = classes.find((c) => c.id === parseInt(attendanceClassId));
      if (!cls) return;
      ...
      Object.keys(dayAttendance).forEach((studentName) => {
        const record = dayAttendance[studentName];
        exportData.push({
