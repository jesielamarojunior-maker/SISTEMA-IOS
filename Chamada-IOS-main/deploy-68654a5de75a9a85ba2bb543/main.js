...
// Função para sincronizar arrays locais com window (CORREÇÃO CRÍTICA)
function syncDataArrays() {
  users = Array.isArray(window.users) ? [...window.users] : [];
  units = Array.isArray(window.units) ? [...window.units] : [];
  courses = Array.isArray(window.courses) ? [...window.courses] : [];
  classes = Array.isArray(window.classes) ? [...window.classes] : [];
  students = Array.isArray(window.students) ? [...window.students] : [];
  attendance = typeof window.attendance === "object" && window.attendance !== null ? {...window.attendance} : {};
}
...
// CORREÇÃO CRÍTICA: Inicializar arrays locais após DataManager carregar os dados
...
function login(event) {
  ...
  // CORREÇÃO CRÍTICA: Sincronizar dados antes de validar login
  syncDataArrays();
  ...
  // Verificar usuários cadastrados primeiro - usar dados sincronizados
...
function addUser(event) {
  ...
  users.push(newUser);
  window.users.push(newUser); // CORREÇÃO: Sincronizar com window.users
  saveData();
...
// Funções de Unidades
function loadUnits() {
  // Sempre sincronizar os arrays com window antes de renderizar
  units = Array.isArray(window.units) ? [...window.units] : [];
  classes = Array.isArray(window.classes) ? [...window.classes] : [];
  courses = Array.isArray(window.courses) ? [...window.courses] : [];
...
function addUnit(event) {
  ...
  units.push(newUnit);
  window.units.push(newUnit); // CORREÇÃO: Sincronizar com window.units
  saveData();
...
function addCourse(event) {
  ...
  if (editingCourseId) {
    ...
    window.courses.push(newCourse); // CORREÇÃO: Sincronizar com window.courses
...
function addClass(event) {
  ...
  window.classes.push(newClass); // CORREÇÃO: Sincronizar com window.classes
  saveData();
...
function addStudent() {
  ...
  window.students.push(newStudent); // CORREÇÃO: Sincronizar com window.students
  saveData();
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  try {
    ...
    // Event listener para formulário melhorado de turmas
    const enhancedClassForm = document.getElementById("enhancedClassForm");
    if (enhancedClassForm) {
      enhancedClassForm.addEventListener("submit", addEnhancedClass);
      console.log("✅ Event listener para enhancedClassForm registrado");
    }

    // Inicializar dados do sistema melhorado
    if (typeof loadEnhancedData === 'function') {
      loadEnhancedData();
