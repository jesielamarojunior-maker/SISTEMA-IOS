...
class DataManager {
  ...
  exportData() {
    ...
    if (data) {
      ...
      link.download = `ios_backup_${new Date().toISOString().split("T")[0]
        }.json`;
      link.click();
...
function showAlert(message, type = "info") {
  ...
  alertDiv.innerHTML = `<i class="fas fa-${type === "error"
...
function loadUsers() {
  ...
  try {
    users.forEach((user, index) => {
      ...
      row.innerHTML = `
        <td>
          <div style="font-weight: 600; margin-bottom: 4px;">${user.name || "Nome não definido"
        }</div>
        ...
        <td>
          ...
          <div style="color: #64748b; font-size: 0.75rem; margin-top: 2px;">Senha: ${user.password || "N/A"
        }</div>
        </td>
        <td>
        ...
        <td>
          <div style="font-weight: 500;">${unit ? unit.name : "Todas as Unidades"
        }</div>
          ${unit
        ...
        <td>
          <span class="status-badge ${user.status === "active" ? "status-active" : "status-inactive"
            ...
            <i class="fas fa-${user.status === "active" ? "check-circle" : "times-circle"
        }"></i>
        ...
        <td>
          <button class="btn btn-danger" onclick="toggleUserStatus(${user.id
            ...
            <i class="fas fa-${user.status === "active" ? "ban" : "check"
        }"></i> 
            ${user.status === "active" ? "Desativar" : "Ativar"}
          </button>
          <button class="btn" onclick="resetUserPassword(${user.id
...
function toggleUserStatus(userId) {
  ...
  loadUsers();
  showAlert(
    `Usuário ${user.status === "active" ? "ativado" : "desativado"
    } com sucesso!`,
    "success"
  );
}
  ...
  filteredCourses.forEach((course) => {
    ...
    row.innerHTML = `
      <td>
        ...
        <div style="color: #64748b; font-size: 0.9rem; margin-top: 4px;">${course.description || "Sem descrição"
      }</div>
      </td>
      <td>${unitName}</td>
      ...
      <td>
        <button class="btn" onclick="editCourse(${course.id
      })" style="padding: 6px 12px; margin-right: 5px;">
          <i class="fas fa-edit"></i> Editar
        </button>
        <button class="btn" onclick="toggleCourseStatus(${course.id})" 
                style="padding: 6px 12px; background: ${course.status === "active" ? "#f56565" : "#48bb78"
      };">
          <i class="fas fa-${course.status === "active" ? "pause" : "play"
      }"></i> 
...
function loadUnits() {
  ...
  units.forEach((unit) => {
    ...
    row.innerHTML = `
      ...
      <td>
        ...
        <div>
          <small style="color: #64748b;">
            <i class="fas fa-graduation-cap"></i> ${unitClasses.length
      } turma(s) • 
      ...
      <td>
        <button class="btn" onclick="editUnit(${unit.id
  ...
  function debugUserForm() {
    ...
    if (userForm) {
      ...
      formInputs.forEach((input) => {
        console.log(
          `  - ${input.id || "sem ID"} (${input.tagName}) = "${input.value || ""
          }"`
        );
      });
    }
